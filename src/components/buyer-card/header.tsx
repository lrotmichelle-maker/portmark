...
export function Header({ data, isInactive }: HeaderProps) {
    ...
    // Status Badges retain their vibrant identity rules
                ...
                <div className="flex items-center gap-3">
                    ...
                    <div className="flex flex-col min-w-0">
                        <div className="flex items-center gap-1">
                            <h3 className={`text-sm font-black truncate tracking-tight transition-colors duration-300 ${isInactive ? 'text-zinc-500' : 'text-white'}`}>{data.sellerName}</h3>
                            {/* Icon stays un-grayed and vibrant */}
                            <CheckCircle className="w-3.5 h-3.5 text-amber-500 fill-amber-500/10 shrink-0" strokeWidth={2.5} />
                        </div>
                        <span className="text-xs text-zinc-500 font-medium truncate">@{data.sellerUsername}</span>
                    </div>
                </div>
