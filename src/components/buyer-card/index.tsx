...
export default function BuyerCard({
  ...
  const [overpaymentMessage, setOverpaymentMessage] = useState<string | null>(
    null
  );
  ...
  const baselinePriceRaw =
    parseFloat(priceState.productPrice.replace(/[^0-9.]/g, '')) || 0;
  const activeSellerPriceRaw =
    parseFloat(priceState.sellerCounterOffer.replace(/[^0-9.]/g, '')) || 0;
  const currentTargetPrice = priceState.isCountered
    ? activeSellerPriceRaw
    : baselinePriceRaw;
  ...
  const handleInputChange = (value: string) => {
    ...
    if (enteredVal < minimumFloorLimit) {
      setValidationError(
        `Minimum offer $${minimumFloorLimit.toFixed(2)} (${ruleLabel})`
      );
      return;
    }
  ...
  const handleAcceptAction = () => {
    ...
    const currentActiveStatus =
      customStatus ||
      (priceState.isCountered ? 'Counter Offer' : 'Standard');
  ...
  const handleCounterSubmit = (e: React.FormEvent) => {
    ...
    setPriceState((prev) => {
      const oldBaselinePrice = prev.isCountered
        ? prev.sellerCounterOffer
        : prev.productPrice;
      return {
        ...prev,
        isCountered: true,
        buyerOriginalOffer: oldBaselinePrice,
        sellerCounterOffer: formattedPrice,
      };
    });
  ...
  const finalPayableAmount = priceState.isCountered
    ? priceState.sellerCounterOffer
    : priceState.productPrice;
  const isInactive =
    customStatus === 'Lapsed' || customStatus === 'Finalized';
