    ...
    <div className="bg-zinc-950/20 border border-neutral-900 rounded-2xl p-3.5 flex flex-col gap-4">
      <div className="flex flex-col gap-2 px-0.5">
        <p className={`text-[11px] leading-relaxed font-normal transition-colors duration-300 ${isInactive ? 'text-zinc-500' : 'text-zinc-400'}`}>
          {data.description || data.orderDescription}
        </p>
        ...
        <div className="grid grid-cols-2 gap-3">
          <div className="flex items-center gap-3 bg-[#0B0B0B]/90 border border-neutral-800/80 rounded-xl px-3 py-2.5 shadow-sm">
            ...
            <div className="flex flex-col">
              <span className="text-[9px] text-zinc-500 font-bold uppercase tracking-wider leading-none mb-1">Followers</span>
              <span className={`text-xs md:text-sm font-black font-mono tracking-tight leading-none transition-colors duration-300 ${isInactive ? 'text-zinc-500' : 'text-emerald-400'}`}>{formatCompactValue(Math.max(data.followers ?? 3000, 3000))}</span>
            </div>
          </div>
          ...
          <div className="flex items-center gap-3 bg-[#0B0B0B]/90 border border-neutral-800/80 rounded-xl px-3 py-2.5 shadow-sm">
            ...
            <div className="flex flex-col">
              <span className="text-[9px] text-zinc-500 font-bold uppercase tracking-wider leading-none mb-1">Value</span>
              <span className={`text-xs md:text-sm font-black font-mono tracking-tight leading-none transition-colors duration-300 ${isInactive ? 'text-zinc-500' : 'text-emerald-400'}`}>{formatCompactValue(Math.max(data.value ?? 40, 40))}</span>
            </div>
          </div>
        </div>
        ...
        <div className="flex items-center justify-between pt-3 border-t border-neutral-900/80 px-0.5">
          <div className="flex items-center gap-1.5">
            ...
            <div className="flex items-center gap-[2px] text-[10px]">
              ...
              <span className={`font-black font-mono tracking-tight transition-colors duration-300 ${isInactive ? 'text-zinc-500' : 'text-white'}`}>{Math.round(data.erCurrentRatio ?? 0)}:1</span>
            </div>
          </div>

          <div className="flex items-center gap-1.5">
            <Heart className="w-3.5 h-3.5 text-red-500 fill-red-500/10 shrink-0" strokeWidth={2.5} />
            <span className={`text-[10px] font-black font-mono tracking-tight transition-colors duration-300 ${isInactive ? 'text-zinc-500' : 'text-white'}`}>{formatCompactValue(Math.max(data.likes ?? 12000, 12000))}</span>
          </div>
          ...
          <div className="flex items-center gap-1.5">
            ...
            <div className="flex items-center gap-[2px] text-[10px]">
              ...
              <span className={`font-black font-mono tracking-tight transition-colors duration-300 ${isInactive ? 'text-zinc-500' : 'text-white'}`}>{Math.round(data.vlCurrentRatio ?? 0)}:1</span>
            </div>
          </div>
        </div>
      </div>
